...
contract BondingCurve {
    ...
    constructor(uint256 _fundingSupply, uint256 _fundingGoal) {
        // Calculate B = ln(FUNDING_GOAL) / FUNDING_SUPPLY
        B = uint256(int256(_fundingGoal).lnWad()).divWad(_fundingSupply);

        // Calculate A = (FUNDING_GOAL * B) / (exp(B * FUNDING_SUPPLY) - 1)
        uint256 exp_b_supply = uint256(int256(B.mulWad(_fundingSupply)).expWad());
        A = _fundingGoal.fullMulDiv(B, exp_b_supply - 1);
    }
...
contract SeiPAD is ReentrancyGuard, Ownable {
    enum TokenState {
        NOT_CREATED,
        FUNDING,
        TRADING
    ...
    uint256 public constant ROUND_OFF = 100000000000000;
    ...
    event Buy(address indexed token, uint256 timestamp, uint256 ethAmount, uint256 tokenAmount);
    event Sell(address indexed token, uint256 timestamp, uint256 ethAmount, uint256 tokenAmount);
    ...
    constructor(
        ...
        bondingCurve = new BondingCurve(FUNDING_SUPPLY, FUNDING_GOAL);
    ...
    function claimToken(address token, uint256 amount) external onlyOwner {
        IERC20(IERC20(token)).transfer(msg.sender, amount);
    }
    ...
    function createToken(
        ...
        require(msg.value >= creationFees, "creation fees not paid");
        fee += msg.value;
        Token tokenAddress = new Token(name, symbol);
        tokens[address(tokenAddress)] = TokenState.FUNDING;
        emit TokenCreated(address(tokenAddress), block.timestamp);
    ...
    function buy(address tokenAddress) external payable nonReentrant {
        ...
        emit Buy(tokenAddress, block.timestamp, contributionWithoutFee, amount);
        // when reached FUNDING_GOAL
        if ((tokenCollateral + ROUND_OFF) >= FUNDING_GOAL) {
            token.mint(address(this), MAX_SUPPLY - token.totalSupply());
    ...
    function sell(address tokenAddress, uint256 amount) external nonReentrant {
        ...
        emit Sell(tokenAddress, block.timestamp, receivedETH, amount);
        // send ether
    ...
    function burnLiquidityToken(address pair, uint256 liquidity) internal {
        IERC20(IERC20(pair)).transfer(address(0), liquidity);
    }
